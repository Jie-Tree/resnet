...
from managedata import load_staging_data
import numpy as np
import resnet
import datetime
...
(X_train, y_train_staging), (X_test, y_test_staging) = load_staging_data()
...
resnetnum = 101
Staging_model = resnet.ResnetBuilder.build_resnet_101((img_channels, img_rows, img_cols), nb_classes)
Staging_model.compile(loss=myloss,
              optimizer=opt,
              metrics=['accuracy'])

print(X_train.shape, Y_train_staging.shape)
print(X_test.shape, Y_test_staging.shape)
...
if not data_augmentation:
    ...
    Staging_model.fit(X_train, Y_train_staging,
              ...
              nb_epoch=nb_epoch,
              # validation_data=(X_test, Y_test_staging),
              validation_split=0.15,  # (X_test, Y_test),
              shuffle=True,
              callbacks=[lr_reducer, early_stopper, Staging_history])
