...
from managedata import load_grading_data
import numpy as np
import resnet
import datetime
...
(X_train, y_train_grading), (X_test, y_test_grading) = load_grading_data()
...
tr_mean_image = np.mean(X_train, axis=0)
te_mean_image = np.mean(X_test, axis=0)

X_train -= tr_mean_image
X_test -= te_mean_image
X_train /= 128.
X_test /= 128.

resnetnum = 101
Grading_model = resnet.ResnetBuilder.build_resnet_101((img_channels, img_rows, img_cols), nb_classes)
Grading_model.compile(loss=myloss,
              optimizer=opt,
              metrics=['accuracy'])

print(X_train.shape, Y_train_grading.shape)
print(X_test.shape, Y_test_grading.shape)
...
if not data_augmentation:
    ...
    Grading_model.fit(X_train, Y_train_grading,
              ...
              nb_epoch=nb_epoch,
              # validation_data=(X_test, Y_test_grading),
              validation_split=0.15,  # (X_test, Y_test),
              shuffle=True,
              callbacks=[lr_reducer, early_stopper, Grading_history])
